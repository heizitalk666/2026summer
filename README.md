...
tests/          507 条用例（394 个测试函数，参数化展开后 507）
```
...
| 项目 | 状态 |
|---|---|
| 边缘端四进程 + 云端台账 | 全链路跑通；三轮 300 s 连续运行无进程退出，复核成功率 71.4 / 87.5 / 100 %（中位 87.5，目标 > 85） |
...
| 测试 | 507 条用例，`validate` 57 项全绿 |
| YOLO 权重 | ✅ **两级真权重已接进全链路**（`cruise_ft` / `verify_ft`，`detector: yolo`）。无泄漏真实 val：mAP50 0.9941、漏检 0.168 / 0.185 %；RTX 3060 单帧 22 / 43 ms @1280 |
| RKNN 上板 | 转换与 INT8 掉点**已完成**（相对 L1 5.4 / 5.0 %，见 `deliverables/丙-异常/rknn/`）；只剩上板测速等硬件 |

**重复性这一项超差，43 次独立标定里 29 次不合格——是系统性的，不是浮动。**
原先归因于「合成检测器注入的检测框抖动」，实测**不成立**：标定路径上 2450 次读数的
像素密度全是同一个值 149.460，根本没有检测框抖动。真正的来源是逐点的角度依赖
（五个标定点的偏离差近 8 倍，最差的是指针指向正上方那一点）。

另有一类**量级失控**：6 次读数（0.245 %）偏离该点中位 2.0–14.7°，单独把所在那轮的
重复性推到 3.7–5.5 % FS。它 100 % 可检出——正常读数的 `confidence` 全部恰好是
1.000，失控的全部 ≤ 0.879。

**不调参数把它凑过去**，那等于把误差藏起来；真机上同样的失效还会发生，只是那时
没人知道它曾在仿真里出现过。逐条见

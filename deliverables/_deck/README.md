# 中期答辩 PPT 的源

```bash
cd deliverables
npm ci                                # 按 package-lock.json 装，版本钉死（^4.0.1）
NODE_PATH=$(pwd)/node_modules node make_deck.js   # → deliverables/中期答辩.pptx
cd .. && python deliverables/_deck/qa_layout.py deliverables/中期答辩.pptx   # 版面自检，必跑
```

> **版本钉死是有原因的**：pptxgenjs 换个版本，字号度量与自动换行会变，
> 一页排得下的字可能就排不下了——`qa_layout.py` 会抓到，但那时已经在改版式了。
> 首次装用 `npm install pptxgenjs` 生成 lock 文件；之后一律 `npm ci`。
> `node_modules/` 不进版本库（`.gitignore` 已排除）。

版式取自学院模板 `C424_PPT_2024`：顶部蓝色横幅、右上校徽、底部细条、左上「一.节名」
标签、居中的白色圆角二级标题块。模板套版图切在 `img/tpl/`，几何尺寸与模板逐项对齐。

## 文件

| 文件 | 内容 |
|---|---|
| `../make_deck.js` | 入口：设版面（**LAYOUT_WIDE 必须在 addSlide 之前设**）、按顺序拼两个正文模块 |
| `theme.js` | 配色、模板套版与组件：`slide()` `head()` `foot()` `toc()` `card()` `stat()` `table()`。**页码自动递增** |
| `deck.js` | 第 1–16 页：封面、目录、**一 研究背景**、**二 关键难题与研究目标**、**三 前半**（3.1 总体方案、3.2 决策链、3.3 复核流程、3.4 系统结构、3.5 数据流、3.6 虚拟试验台、3.7 资料来源） |
| `deck2.js` | 第 17–28 页：**三 后半**（3.8 识别层、3.9 L1、3.10 L2、3.11 L3、3.12 比选汇总、3.13 安全边界、3.14 云台与闭环）、**四 项目特色**、**五 计划与文献**、结束页 |
| `qa_layout.py` | 版面自检：出界 / 底部留白 / 元素重叠 / 换行溢出。改完必跑 |
| `img/` | 嵌入的图；`img/tpl/` 是从模板里切出来的套版图 |

**页码不用手工维护。**所有页必须用 `T.slide(pres)` 创建，`T.foot(s, "说明")` 不传页码。
分节页用 `T.toc(pres, 节号, "本节说明")`，当前节自动高亮。

## 第三部分每页都有一行「决定行」

`T.head(s, 节名, 标题, sub)` 的第四个参数传数组 `[采用什么, 为什么]` 时，
副标题位置会排成一行绿色的结论加灰色的理由：

```js
T.head(s, "三.复核流程", "3.3　什么时候该停车复核……",
  ["十状态机 + 复核预算 N_max + 三条抑制规则",
   "停一次车就少走一段路，必须算得出这一趟还停得起几次，且不在同一处反复停"]);
```

第三部分（3.3 起）每一页都用这种写法。翻到哪一页，先读到的都是这一环选了什么、
为什么选它，指标退到证据的位置。传字符串则仍是原来的普通副标题，
第一、二、四、五部分照旧。

**3.2 决策链是第三部分的索引页**：一张表五列——任务书列出的约束 / 我们的做法 /
为什么这么做 / 怎么验证它成立 / 见页。改动后面任何一页的编号，记得同步这一列，
以及 3.1 页三个子问题卡片右上角的「见 3.x」。

## 版面上踩过的两个坑

1. **表格的 `height` 是名义值。**pptxgenjs 写给表格 graphicFrame 的 `height` 恒为 1.0"，
   真实高度要按行高求和；渲染器还会按内容再撑高行。靠 python-pptx 读 `shape.height`
   判重叠会全部漏掉——`qa_layout.py` 因此自己按行高与换行重算表格高度。
2. **图片素材直接决定文件大小。**pptxgenjs 不去重，同一张图用在 N 页就存 N 份。
   套版图按 150 DPI 的实际显示尺寸重采样、无透明通道的转 JPEG 之后，
   成品由 63 MB 降到 9 MB。目录页用的是单独裁切的 `toc_photo.jpg`，
   直接拉伸整幅封面图会把 2:1 的照片压成 1:1。

## 待补槽位已全部撤掉

两处「待补充」的琥珀色虚线框都没有了，`T.slot()` 也从 `theme.js` 删了：

- 乙 的核心比选图已交（`reading_error.png`，n = 200、P90、bootstrap 95 % CI），
  第 18 页整页重写为读数方案的实测比选。
- 甲 的 L1 复核级来不及做，框子按组长要求撤掉。这三项（复核级 yolo11m、
  单帧耗时、漏检率）没有消失，在第 26 页的进度表里标着「进行中」，
  答辩被问到按那一页答，不在 L1 页展开。

要是后面又需要这种框，`git show be6b26a:deliverables/_deck/theme.js` 里有原来的实现。

## 改完必跑

```bash
NODE_PATH=$(pwd)/node_modules node make_deck.js
python3 _deck/qa_layout.py 中期答辩.pptx                          # 版面自检
python3 <pptx-skill>/scripts/office/validate.py 中期答辩.pptx     # 文件结构
python3 <pptx-skill>/scripts/office/soffice.py --headless --convert-to pdf 中期答辩.pptx
pdftoppm -jpeg -r 100 中期答辩.pdf slide                          # 逐页看
```

沙箱里 LibreOffice 默认只装 core，要 `apt-get install libreoffice-impress poppler-utils`
才能转 PDF 和出图。

## 页脚只放答辩现场用得上的东西

`T.foot(s)` 可以不传说明，只出页码。凡属下面这几类的，一律不写进页脚：

- **解释这一页怎么读**——「本页是第三部分的索引，后面每一页展开其中一行」
- **声明自己没作弊**——「本页全部内容引自任务书，未作增补」「现状调研为本组自行整理」
- **对着评分表自评**——「五个环节均比选 ≥ 2 种方案」「任务书列出的参考资料五类均已覆盖」
- **报代码行数**——「网关 697 行」「fsm.py 598 行十状态」

这些是写给自己看的，讲给评审只会占地方。留下的只有三类：复现命令、评测条件、
代码位置。正文里同类的自我说明也照此处理，比如比选汇总页的收尾卡原来在数
比选了几个环节，改成了讲那两行「不采用」的结论是怎么来的。

## 字号

正文整体比初版大一档：最小 9 pt（原 7.8），表格正文 10 至 10.8 pt，
卡片标题 11.5 至 13 pt，页脚 9.8 pt。行距同步放大，否则行会挤在一起。

调字号会连带撑高表格。**改完一定要跑 `qa_layout.py`**：它按行高与换行重算
表格实际高度，能查出被下方卡片压住的末行。上一轮整体调大之后，
3.1、3.4、3.5、3.8 四页都因此需要重新排位置。

## 内容口径

- **任务书只有一份**：`22 基于RK3576边缘计算的无人车主动式AI巡检系统设计.docx`。
  它规定的是能力要求，**没有给出任何数值指标**。PPT 里凡是数值指标，
  页脚都注明「本组设定」，不能写成任务书要求。
- 页面上的数字**全部是本次实测**，不用文档旧值。两处与旧文档不一致：
  - PID：旧 README 写 0.9 % / 1.10 s / 47.6 %；本次实测
    **3.0 % @1× · 1.0 % @3× · 关调度 37.7 %**
  - 端到端复核成功率：不写「100 %」，如实写 **75–83 %**（未达 85 % 目标）并说明主因
- 讲述重心是**采用的方案与选择理由**，不是工作量统计。第 9 页（3.1）是全篇主线：
  一句话说清方案，再讲否定掉的两种备选和否定理由，最后引出三个子问题。
- 封面汇报人写的是**吴明哲**，第 3 页八项研究内容用的是**任务书原文的分号列举**，
  结束页**没有**「现场可演示」那一行——这三处是组长在 PowerPoint 里手改后
  反馈回来的，已经收进脚本，重跑不会被覆盖，别再改回去。
- 字体是**微软雅黑**。中间试过公文那套「标题黑体、正文仿宋」，组长看过之后
  决定还用雅黑，不要再换。

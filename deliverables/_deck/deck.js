...
module.exports = function (pres, IMG) {
  ...
  // ============================================================ 总体方案与选择理由
  {
    ...
    s.addText("方案三定下来之后，剩下的全部研究内容就是三个必须回答的子问题——本篇第三部分按这条主线展开", {
      ...
      color: C.navy, isTextBox: true, margin: 0, valign: "middle" });
    const Q = [
      ["什么时候该停车复核", "十状态机 + 复核预算 + 三条抑制规则", "3.2",
       "停一次车就少走一段路。必须算得出这一趟还停得起几次，并且不会在同一处反复停车。", C.blue],
      ["停下来怎么把目标放大到位", "针孔几何前馈 + 变焦增益调度 PID", "3.13",
       "变焦后同样的云台转角对应的画面位移成倍放大，固定增益的回路必然过冲，对不准就谈不上放大。", C.red],
      ["放大之后凭什么下结论", "四类模型分工 + 显式规则仲裁", "3.7、3.11",
    ...
    T.foot(s, "总体方案的比选依据同样包含非技术因素：方案一的续航与带宽成本、方案二的断网可靠性，均在否定理由中");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.复核流程", "3.2　主动复核状态机：十状态与三条抑制规则",
      "对应任务书研究内容第 5 项：停车、居中、变焦、多角度补拍与恢复巡检路线");
    const steps = [
      ["CRUISE", "30 Hz 巡航\nL1 小模型扫描", C.blue],
      ["SUSPECT", "连续三帧同一目标\n才确认，防抖动", C.blue],
      ["HALT_REQ", "下发「暂停巡检」\n网关五项校验", C.red],
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.系统结构", "3.3　系统结构：四进程、四接口与五份接口契约",
      "进程边界按安全职责划分：安全网关必须在感知与任务进程异常退出后继续工作");
    const procs = [
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.实施路径", "3.4　一次完整复核的十二步数据流",
      "跨四个边缘进程与云端。这条链路完整走通，各层接口即为正确");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究方法", "3.5　支撑性方法：虚拟试验台与接口冻结",
      "硬件到位时间不确定，但控制逻辑与接口联调不应等待硬件");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.资料来源", "3.6　数据从何获取，如何处理",

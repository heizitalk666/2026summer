...
module.exports = function (pres, IMG) {
  ...
  // ============================================================ 识别层
  {
    ...
    [["在哪", "画面里有哪些设备，框在哪", "目标检测 L1", "需全图扫描且延迟低，巡航期 30 Hz 只负担得起这一路"],
     ...
     ["信谁", "四路证据矛盾时如何取舍", "显式规则仲裁 L4", "结论要能逐条给出判定依据，模型输出的置信度做不到这一点"],
      ...
      T.td(r[1], { fontSize: 10.5 }), T.td(r[2], { bold: true, color: C.blue, fontSize: 10.5 }),
      T.td(r[3], { color: C.muted, fontSize: 10.5 })]));
    ...
    V.forEach(([n, cn, col], i) => {
      ...
      s.addText(n, { x: M + 8.78, y, w: 2.35, h: 0.36, fontFace: F, fontSize: 10.2,
        bold: true, color: C.text, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(cn, { x: M + 11.16, y, w: 1.10, h: 0.36, fontFace: F, fontSize: 10.2,
    ...
    L.forEach(([t, d], i) => {
      const y = y0 + 3.62 + i * 0.62;
      T.card(s, M, y, W - M * 2, 0.56);
      s.addText(t, { x: M + 0.22, y, w: 3.05, h: 0.56, fontFace: F, fontSize: 11.2,
        bold: true, color: C.navy, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(d, { x: M + 3.42, y, w: 8.95, h: 0.56, fontFace: F, fontSize: 10,
        color: C.muted, isTextBox: true, margin: 0, valign: "middle", lineSpacing: 13.5 });
    });
  ...
  // ============================================================ L1
  {
    ...
    st.forEach(([v, k, col], i) => {
      const x = M + i * 2.20;
      T.card(s, x, y0, 2.06, 1.10);
      T.stat(s, x + 0.20, y0 + 0.10, 1.7, v, k, col);
    });
    T.card(s, M + 8.86, y0, 3.52, 1.10, C.amberSoft);
    s.addText("第 1 轮 mAP50 已达 0.9759，偏高。\n需先排除训练集与验证集之间的同源增广副本。", {
      x: M + 9.06, y: y0, w: 3.16, h: 1.10, fontFace: F, fontSize: 11.2, bold: true,
      color: "8A5200", isTextBox: true, margin: 0, valign: "middle", lineSpacing: 15.5 });

    // 图片按原始比例摆：PR 曲线 3:2，混淆矩阵 4:3
    s.addImage({ path: IMG + "/l1_pr.png", x: M, y: y0 + 1.26, w: 3.60, h: 2.40 });
    s.addImage({ path: IMG + "/l1_cm.png", x: M + 3.76, y: y0 + 1.26, w: 3.20, h: 2.40 });

    T.card(s, M + 7.12, y0 + 1.26, 5.26, 2.40);
    T.cardTitle(s, M + 7.36, y0 + 1.38, 4.8, "0.25 与 0.60 这两个阈值是怎么定的");
    s.addText("0.25 是巡航级的下限。再低，抑制规则挡不住的误触发会耗尽复核预算。\n" +
      ...
      x: M + 7.36, y: y0 + 1.76, w: 4.82, h: 1.80, fontFace: F, fontSize: 11.2,
      color: C.muted, isTextBox: true, margin: 0, valign: "top", lineSpacing: 16.5, paraSpaceAfter: 5 });

    T.card(s, M, y0 + 3.82, W - M * 2, 0.94, C.blueSoft);
    s.addText("方案比选：合成检测器 vs 公开集训练的 YOLO　|　" +
      ...
      x: M + 0.28, y: y0 + 3.82, w: 11.85, h: 0.94, fontFace: F, fontSize: 11.5,
      color: C.navy, isTextBox: true, margin: 0, valign: "middle", lineSpacing: 16.5 });
    T.foot(s, "任务书点名可用 YOLO 系列与 MobileNet，本组选 YOLO11");
    s.addNotes("L1 巡航级已训完，指标偏高。第 1 轮就到 0.9759 这个数不正常，我们自己先提出来：已写好 --check-" +
      "leak 命令排查训练集与验证集的同源增广副本。复核级 yolo11m、单帧耗时与漏检率这三项本阶段没有做完，" +
      "第 26 页的进度表里如实标着，被问到就按那一页答，不在这一页展开。");
  }
  ...
  // ============================================================ L2
  {
    ...
    s.addText("灰底两档为像素密度低于 96 px 的区间。fusion.py 的 DENSITY_FLOOR_FRAC = 0.80 " +
      ...
      x: M, y: y0 + 4.16, w: 7.30, h: 0.44, fontFace: F, fontSize: 9.8,
    ...
    s.addText("门槛以上三档，几何法 P90 稳定在 0.18 至 0.19 %FS，U-Net 为 0.36 至 0.40，" +
      ...
      x: M + 7.74, y: y0 + 3.30, w: 4.42, h: 1.22, fontFace: F, fontSize: 9.8,
      color: C.navy, isTextBox: true, margin: 0, valign: "top", lineSpacing: 13.5 });
    ...
    s.addText("采用几何解算的三条理由　①　门槛以上三档误差最低且置信区间不重叠；" +
      ...
      x: M + 0.28, y: y0 + 4.70, w: 11.85, h: 0.92, fontFace: F, fontSize: 10,
  ...
  // ============================================================ L3
  {
    ...
    [["误报率（正常裁片 106）", "1.9 %", "0.0 %", "3.8 %", "3.8 %"],
     ...
     ["可解释（伦理要求）", "可说明哪个通道", "否", "否", "否"],
      ...
      T.td(r[0], { bold: true, fontSize: 10.2 }),
      T.td(r[1], { align: "center", fontSize: 10.2 }),
      T.td(r[2], { align: "center", fontSize: 10.2, color: C.muted }),
      T.td(r[3], { align: "center", fontSize: 10.2 }),
      T.td(r[4], { align: "center", fontSize: 10.2, bold: true,
    ...
    why.forEach(([t, d, col], i) => {
      ...
      s.addText(t, { x: M + 0.20, y, w: 2.35, h: 0.60, fontFace: F, fontSize: 10.5,
        bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(d, { x: M + 2.66, y, w: 4.80, h: 0.60, fontFace: F, fontSize: 9.5,
    ...
    s.addText("1　训练裁片必须模拟运行时的检测框噪声。实测裁掉 12 % 的正常表盘，异常分由 0.5 升到 1.0，全部误报。" +
      ...
      x: M + 8.02, y: y0 + 3.62, w: 4.14, h: 1.48, fontFace: F, fontSize: 9.8,
      color: C.navy, isTextBox: true, margin: 0, valign: "top", lineSpacing: 13.5 });
  ...
  // ============================================================ 比选汇总
  {
    ...
    [["L1 检测", "① 合成检测器\n② YOLO11 公开集训练", "① 零成本可离线复现但纹理单一\n② 纹理多样但需 GPU 工时",
     ...
     ["部署形态", "① 云端推理\n② 边缘推理（RK3576）", "① 算力充足\n② 受 NPU 算力限制",
      ...
      T.td(r[1], { fontSize: 10 }), T.td(r[2], { fontSize: 10 }),
      T.td(r[3], { fontSize: 10, color: C.red }),
      T.td(r[4], { fontSize: 10, bold: true, color: C.green })]));
    ...
    s.addText("表里有两行的结论是「不采用」：L3 的 EfficientAD 复现后分数倒挂，L2 的 U-Net 读数误差反而更大。" +
      ...
      x: M + 0.28, y: y0 + 4.74, w: 11.85, h: 0.62, fontFace: F, fontSize: 10.5,
      color: "DCE9F5", isTextBox: true, margin: 0, valign: "middle", lineSpacing: 14.5 });
    T.foot(s);
  ...
  // ============================================================ 安全边界
  {
    ...
    s.addText("任务书允许的高层指令：暂停巡检　·　低速前进　·　移动至观察点　·　云台转向 / 变焦　·　恢复路线　" +
      "　　急停、避障、限速的优先级高于巡检任务", {
      x: M + 0.28, y: y0, w: 11.85, h: 0.72, fontFace: F, fontSize: 11.5, bold: true,
    ...
    layers.forEach(([t, col, d, cmd, res], i) => {
      ...
      s.addText(t, { x: M + 0.26, y: y + 0.08, w: 5.5, h: 0.30, fontFace: F, fontSize: 12.5,
        bold: true, color: C.text, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(d, { x: M + 0.26, y: y + 0.38, w: 5.6, h: 0.66, fontFace: F, fontSize: 10,
        color: C.muted, isTextBox: true, margin: 0, valign: "top", lineSpacing: 13.5 });
      T.card(s, M + 6.10, y + 0.12, 6.28, 0.38, C.blueSoft);
      s.addText(cmd, { x: M + 6.24, y: y + 0.12, w: 6.0, h: 0.38, fontFace: "Courier New",
        fontSize: 9.2, color: C.navy, isTextBox: true, margin: 0, valign: "middle" });
      s.addText("现场可见结果：" + res, { x: M + 6.10, y: y + 0.56, w: 6.28, h: 0.48, fontFace: F,
        fontSize: 10.8, bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
    });
    T.card(s, M, y0 + 4.52, W - M * 2, 0.52, C.blueSoft);
    s.addText("所有指向底盘与云台的动作只经过 gateway 一个出口。其余三个进程即使写错代码，也没有可以调用的驱动实例。", {
      x: M + 0.28, y: y0 + 4.52, w: 11.85, h: 0.52, fontFace: F, fontSize: 11.2, bold: true,
  ...
  // ============================================================ 控制与闭环
  {
    ...
    [["1× 广角", "4.1 %", "0.801 s", "达标"], ["3× 变焦（有调度）", "1.0 %", "1.101 s", "达标"],
     ["3× 变焦（关调度）", "42.7 %", "1.201 s", "不可接受"]].forEach((r, i) => rows.push([
      T.td(r[0], { fontSize: 10.2, bold: i === 2 }),
      T.td(r[1], { fontSize: 10.2, align: "center", bold: true, color: i === 2 ? C.red : C.green }),
      T.td(r[2], { fontSize: 10.2, align: "center" }),
      T.td(r[3], { fontSize: 10.2, align: "center", bold: true, color: i === 2 ? C.red : C.green })]));
    ...
    s.addText("为什么必须做增益调度：同样 1° 的云台转角，在 3× 变焦下对应的画面位移是 1× 的三倍。" +
      ...
      x: M + 6.64, y: y0 + 1.72, w: 5.55, h: 1.00, fontFace: F, fontSize: 10.2,
    ...
    s.addText("复核成功率三轮实测 87.5 %、100 %、71.4 %，中位 87.5 %，第三轮低于本组设定的 85 %。" +
      "跨度来自桩按真机故障率注入的丢包与对焦失败，代码三轮未变——这一项应当按多轮分布判，按单轮判是抽签。", {
      x: M + 0.22, y: y0 + 3.94, w: 5.80, h: 0.72, fontFace: F, fontSize: 10, bold: true,
      color: "8A5200", isTextBox: true, margin: 0, valign: "middle", lineSpacing: 13.5 });
    T.foot(s, "复现：tune_pid --compare-gain-schedule　·　run_all --seconds 300　·　浏览器 127.0.0.1:8000 看台账");
    s.addNotes("课题名称是测控系统，控制部分的实测依据集中在这一页。绿色曲线的过冲是关闭增益调度后的响应。右下角的复核成功率" +
      "三轮跨了 71.4 % 到 100 %，中位刚过目标线，这一条自己先讲：它不是稳定的一个数，而是一个受注入故障率支配的分布。");
  }
  ...
  // ============================================================ 创新点
  {
    ...
    const N = [
      ...
      ["02", "变焦增益调度的云台伺服", C.blue,
       ...
       "对应任务书「云台转向 / 变焦」高层指令",
       "3× 变焦超调 1.0 %、调节时间 1.101 s；关闭调度后超调升至 42.7 %"],
      ...
      ["04", "接口先冻结、驱动先抽象的无硬件开发方法", C.green,
       "五份 Schema 编码前定稿并配 57 项一致性校验；四个抽象基类隔离硬件，桩注入真机上实际存在的故障率。",
    ...
    N.forEach(([n, t, col, d, link, ev], i) => {
      ...
      s.addText(n, { x: M + 0.22, y, w: 0.56, h: 1.10, fontFace: F, fontSize: 20,
        bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(t, { x: M + 0.86, y: y + 0.06, w: 4.90, h: 0.34, fontFace: F, fontSize: 13,
        bold: true, color: C.text, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(d, { x: M + 0.86, y: y + 0.40, w: 4.92, h: 0.62, fontFace: F, fontSize: 9.8,
        color: C.muted, isTextBox: true, margin: 0, valign: "top", lineSpacing: 13.5 });
      T.card(s, M + 5.96, y + 0.10, 2.62, 0.90, C.blueSoft);
      s.addText(link, { x: M + 6.12, y: y + 0.10, w: 2.32, h: 0.90, fontFace: F, fontSize: 10,
        bold: true, color: C.navy, isTextBox: true, margin: 0, valign: "middle", lineSpacing: 13.5 });
      T.card(s, M + 8.74, y + 0.10, 3.64, 0.90);
      s.addText("实测支撑", { x: M + 8.92, y: y + 0.16, w: 3.3, h: 0.24, fontFace: F, fontSize: 10,
        bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(ev, { x: M + 8.92, y: y + 0.40, w: 3.32, h: 0.56, fontFace: F, fontSize: 9.5,
  ...
  // ============================================================ 进度与不足
  {
    ...
    const data = [
      ...
      ["接口一致性与自动化测试", "已完成", "validate 57 项全过；507 项测试，覆盖率 75 %", "无真权重用例"],
      ["安全边界三层", "已完成", "三条演示可现场执行", "无"],
      ["云台 PID 与增益调度", "已完成", "1× 超调 3.0 %，3× 超调 1.0 %，均达标", "故障率与成功率的关系曲线未做"],
      ["L2 几何读数", "已完成", "基本误差 0.455 %、线性度 0.302 % 达标", "重复性 0.309 % 超出 0.3 % 限值"],
    ...
    data.forEach(r => rows.push([
      ...
      T.td(r[1], { align: "center", bold: true, fontSize: 10, color: col[r[1]][0], fill: { color: col[r[1]][1] } }),
      T.td(r[2], { fontSize: 10 }),
      T.td(r[3], { fontSize: 10, color: r[3] === "无" ? C.green : C.muted })]));
    ...
    plan.forEach(([t, d, c], i) => {
      ...
      s.addText(t, { x: M + 0.22, y, w: 3.15, h: 0.60, fontFace: F, fontSize: 10.8,
        bold: true, color: c, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(d, { x: M + 3.52, y, w: 8.85, h: 0.60, fontFace: F, fontSize: 10,
        color: C.muted, isTextBox: true, margin: 0, valign: "middle", lineSpacing: 13.5 });
    });
  ...
  // ============================================================ 参考文献
  {
    ...
    G.forEach(([t, col, items], i) => {
      ...
      s.addText(t, { x: x + 0.22, y: y + 0.06, w: 5.6, h: 0.28, fontFace: F, fontSize: 11.5,
        bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(items.join("\n"), { x: x + 0.22, y: y + 0.36, w: 5.62, h: 1.36, fontFace: F,
        fontSize: 9, color: C.muted, isTextBox: true, margin: 0, valign: "top", lineSpacing: 13 });
    });
    ...
    s.addText("综述如何支撑选题与方案：[1][2][5] 支撑巡航期轻量检测的骨干选择；[3][4] 说明云台转动导致跟踪断链的成因，" +
      ...
      x: M + 0.28, y: y0 + 3.92, w: 11.85, h: 0.80, fontFace: F, fontSize: 10,
      color: "DCE9F5", isTextBox: true, margin: 0, valign: "middle", lineSpacing: 14 });
    T.foot(s);
  ...
  // ============================================================ 结束
  {
    ...
    s.addText("敬请各位老师批评指正", { x: 1.05, y: 4.22, w: 11.23, h: 0.86, align: "center",
      ...
      align: "center", fontFace: F, fontSize: 11.2, color: C.navy, isTextBox: true,
      margin: 0, valign: "middle", lineSpacing: 16.5 });

...
module.exports = function (pres, IMG) {
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.9　L2 读数：三种方案的实测比选与采用理由",
      "对应任务书研究内容第 3 项的图像质量评价与读数环节；核心图取 n = 200 的 P90，误差棒为 bootstrap 95 % 置信区间");
    ...
    s.addNotes("这一页现在能把 L2 的选择讲完整。原来的图只有 24 个样本、画的是中位数，差异被采样噪声盖住，得不出结论；" +
      ...
      "第二，U-Net 的分割 IoU 是三者最高的 0.778，读数误差却更大——因为几何解算只用指针方向，不用完整轮廓，" +
      "IoU 高不等于读数准。最后要老实说：合成表盘天然满足几何法的假设，这个结论换到真实表盘上可能翻过来，所以 U-Net 保留不删。");
  }

...
module.exports = function (pres, IMG) {
  ...
  // ============================================================ 识别层
  {
    ...
    const y0 = T.head(s, "三.研究内容", "3.7　识别层：四类模型的分工与显式仲裁",
      "对应任务书研究内容第 3 项：目标检测、已知缺陷识别、未知异常检测和图像质量评价");
    ...
    [["在哪", "画面里有哪些设备，框在哪", "目标检测 L1", "需全图扫描且延迟低，巡航期 30 Hz 只负担得起这一路"],
     ...
     ["信谁", "四路证据矛盾时如何取舍", "显式规则仲裁 L4", "必须能逐条给出判定依据，这是评审首先会问的"],
    ].forEach(r => rows.push([
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.8　L1 目标检测：训练结果与方案比选",
      "公开数据集 distribution_room（2 773 张，CC BY 4.0），骨干 yolo11s，训练 120 轮");
    ...
    T.cardTitle(s, M + 7.14, y0 + 1.26, 5.0, "为什么分巡航级与复核级两套阈值");
    s.addText("这是「识别准确率↔实时性」这一项取舍的落点。巡航级把阈值压到 0.25 保召回，宁可多报也不漏检；" +
      "误报不在这一层消解，而是留给复核级：阈值提到 0.60，在放大后的画面上重新判定，并输出 Δconf 度量复核带来的增益。", {
    ...
    T.foot(s, "任务书点名可用 YOLO 系列与 MobileNet；本组选 YOLO11，理由见比选汇总页");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.9　L2 语义分割：三级对比与人工核对",
      "对应任务书研究内容第 3 项的图像质量评价与读数环节");
    s.addImage({ path: IMG + "/l2_iou.png", x: M, y: y0, w: 4.75, h: 2.72 });
    s.addImage({ path: IMG + "/l2_mask.png", x: M + 5.22, y: y0, w: 7.16, h: 2.00 });
    s.addText("左：指针 IoU 三级对比。右：分割标注叠加核对图，左半为合成掩膜，右半为 PaddleX 真实标注。" +
      "掩膜错位不会反映在指标上，IoU 仍可训得很高但模型学到的类别对应是错的，因此把人工核对写成硬性验收标准。", {
      x: M + 5.22, y: y0 + 2.06, w: 7.16, h: 0.62, fontFace: F, fontSize: 9,
      color: C.muted, isTextBox: true, margin: 0, valign: "top", lineSpacing: 12 });

    const steps = [["0.251", "纯合成掩膜训练", C.muted], ["0.384", "合成 + PaddleX 真实标注", C.blue],
                   ["0.778", "U-Net（合成 + PaddleX）", C.green]];
    steps.forEach(([v, t, col], i) => {
      const x = M + i * 1.70;
      T.card(s, x, y0 + 2.84, 1.58, 0.84);
      s.addText(v, { x: x + 0.12, y: y0 + 2.88, w: 1.34, h: 0.42, align: "center", fontFace: F,
        fontSize: 17, bold: true, color: col, isTextBox: true, margin: 0, valign: "middle" });
      s.addText(t, { x: x + 0.12, y: y0 + 3.28, w: 1.34, h: 0.36, align: "center", fontFace: F,
        fontSize: 8.2, color: C.text, isTextBox: true, margin: 0, valign: "top", lineSpacing: 10.5 });
    });
    T.slot(s, M + 5.22, y0 + 2.84, 7.16, 0.84, "核心比选图待重画（明日）",
      ["采样量 n=24 提高到 200 以上，纵轴由中位数改为 P90"], "→ 现有采样量不足以支撑比选结论");

    T.card(s, M, y0 + 3.86, W - M * 2, 0.96, C.blueSoft);
    s.addText("方案比选：几何解算 vs numpy 逐像素分类 vs U-Net　|　" +
      "几何法零权重、2.2 ms、可解释；U-Net 权重 31 MB、59 ms、不可解释，但在几何法假设失效的样本上有优势。" +
      "当前结论：合成表盘上三者读数误差同量级且差异小于采样噪声，保持几何法为默认实现，学习法待真实表盘误差表再定。", {
      x: M + 0.28, y: y0 + 3.86, w: 11.85, h: 0.96, fontFace: F, fontSize: 9.8,
      color: C.navy, isTextBox: true, margin: 0, valign: "middle", lineSpacing: 13 });
    T.foot(s, "指针 IoU 由 0.251 升至 0.384 仅因加入真实标注，模型与评测口径不变，说明瓶颈在数据而非模型容量");
    s.addNotes("L2 这一路值得讲的不是 0.778，而是 0.251 到 0.384 这一步：模型不变仅加真实标注就提高 53 %，说明瓶颈在数据。右上那张核对图是数据接入正确的唯一证据，掩膜错位在指标上看不出来。");
  }

  // ============================================================ L3
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.10　L3 未知异常检测：四方案横向比选",
      "非监督方法，训练只用正常样本，避开缺陷标注数据不可得的约束");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.方案比选", "3.11　方案比选汇总：技术与非技术因素的权衡",
    ...
    [["L1 检测", "① 合成检测器\n② YOLO11 公开集训练", "① 零成本可离线复现但纹理单一\n② 纹理多样但需 GPU 工时",
     ...
     ["L2 读数", "① 几何解算\n② numpy 逐像素\n③ U-Net", "① 2.2 ms 零权重\n② 5 ms\n③ 59 ms、31 MB",
      "成本：③ 权重与算力开销最大；可解释：① 每一步可追溯，③ 不可", "暂采用 ①；③ 待真实表盘误差表再定"],
     ["L3 异常", "① 统计法　② EfficientAD\n③ PaDiM 对角　④ PaDiM 全协方差", "漏报 90.8 / 100 / 48.3 / 3.3 %",
    ...
    T.foot(s, "各环节的完整实测数据见前三页；比选记录同步保存在 deliverables/ 各交付目录中");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.12　安全边界：三层防线与现场验证方式",
      "任务书：AI 模块只能发送高层指令，不得直接控制车辆转向、电机扭矩或制动力");
    ...
    const layers = [
      ["第一层　指令白名单与参数硬限", C.red,
       "五项校验逐条上报 PASS、FAIL 或 SKIP。参数范围硬编码在源码中而不放在配置文件里：配置可被修改，安全边界不应该。白名单只含任务书列出的高层指令。",
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "三.研究内容", "3.13　云台伺服与端到端闭环的实测验证",
      "对应任务书研究内容第 5 项与第 8 项：主动复核状态机与运行稳定性测试");
    ...
    s.addText("如实报告：复核成功率两轮实测 83.3 % 与 75.0 %，均低于本组设定的 85 % 目标。" +
    ...
    s.addNotes("课题名称是测控系统，控制部分的实测依据集中在这一页。绿色曲线的过冲是关闭增益调度后的响应。右下角要主动说明复核成功率没达标，原因已定位。");
  }
  ...
  // ============================================================ 进度与不足
  {
    ...
    const data = [
      ...
      ["L2 学习分割", "进行中", "U-Net 指针 IoU 0.778，已接入读数链路", "核心比选图采样量不足"],
    ...
    const plan = [["下一阶段（结题第 3 至 4 周）", "补齐 L1 复核级与切换对比；重画 L2 核心比选图；接入 ByteTrack；RKNN 上板与 INT8 掉点记录；修复证据包配对缺陷；补故障率与复核成功率的可靠性曲线", C.navy],
    ...
    T.foot(s, "状态判定标准：能产出可复现的实测数据才记为已完成；六项已完成、两项进行中、两项未开始");
  ...
  // ============================================================ 结束
  {
    ...
    s.addText("现场可演示：变焦对比　·　越界指令被拒　·　AI 进程终止后车辆走完路线　·　复核 200 ms 中止　·　端到端一轮巡检", {
      x: 1.05, y: 4.92, w: 11.23, h: 0.50, align: "center", fontFace: F, fontSize: 12,

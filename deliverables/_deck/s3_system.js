...
module.exports = function (pres, IMG) {
  // ============================================================ 21 安全边界
  {
    ...
    const y0 = T.head(s, "auto", "安全边界：三层防线，每一条都能当场演示", "按方案书的要求，安全设计需要以可现场复现的方式验证，而不是仅在文档中声明");
    const layers = [
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "auto", "云台控制：变焦增益调度是功能必需项，不是性能优化项", "方案书 §11.1 列出的交付物：云台阶跃响应曲线，本页为本次实测结果");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "auto", "端到端：全系统跑一轮真实巡检", "四个边缘进程与云端台账构成全链路，单轮 300 秒，结束后输出统计小结");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "auto", "风险与未决事项", "如实列出，其中包含两项已定位但尚未修复的缺陷");
  ...
  {
    const s = T.slide(pres);
    const y0 = T.head(s, "auto", "下一阶段计划与分工", "按接口边界划分，四条线可并行推进");
    const plan = [

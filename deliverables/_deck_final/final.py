...
def s19(s):
    ...
    rows = [["考核项", "指标", "虚拟配电室实测", "结果"],
            ...
            ["巡航帧率", "≥ 10 fps", "10 fps；感知处理 P95 36.1 ms", ok],
            ["断网降级", "证据包不丢失", "断网 75 s：4 包全部留在本地待续传", ok],
            ["安全边界", "AI 不直接控制底层执行机构", "三层防线，三条演示可当场执行", ok],
            ["端到端复核成功率", "> 85 %", "真权重一轮 7/7；三轮 180 s 中位 83.3 %", part]]
    table(s, M, 1.12, [2.5, 3.72, 4.75, 1.52], rows, row_h=0.412, size=12, bold_first_col=True, name="table_main")
...
def s20(s):
    ...
    cw = (W - 2 * M - 0.6) / 3
    x = M
    y0 = card(s, x, cy, cw, ch, "连续运行 300 s × 3 轮")
    rows = [["轮次", "证据包", "密度比", "退出"], ["1", "8", "2.22", "无"], ["2", "7", "1.94–2.32", "无"], ["3", "7", "2.11–2.32", "无"]]
    table(s, x + 0.2, y0 + 0.12, [0.62, 0.8, 1.35, 0.8], rows, row_h=0.44, size=12, aligns=["c"] * 4, name="table_runs")
    text(s, x + 0.25, y0 + 2.05, cw - 0.45, 1.05, "四个核心进程每轮跑满 300 s，没有中途退出；驱动层按设定故障率注入。",
         size=12, spacing=1.2, name="card_body")
    x = M + cw + 0.3
    y0 = card(s, x, cy, cw, ch, "复核成功率")
    stat(s, x + 0.25, y0 + 0.02, cw - 0.45, "7 / 7", "真权重一轮，7 个证据包读数全部产出", color=GREEN, vsize=30)
    ...
    for i, (v, l) in enumerate([("59 项", "接口一致性校验全部通过"), ("536 条", "自动化测试用例"), ("36.1 ms", "感知处理 P95（节拍 100 ms）")]):
        stat(s, M + 0.45 + i * 4.15, sy + 0.14, 3.8, v, l, color=NAVY, vsize=28)
    footnote(s, "复现：python -m patrol.tools.run_all --seconds 300；断网测试：run_all --seconds 75 --no-cloud", y=6.86)

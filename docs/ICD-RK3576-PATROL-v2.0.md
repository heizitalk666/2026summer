...
| 版本 | **v2.1**（v2.0 D3 评审通过并冻结；v2.1 见下方修订记录） |
| 状态 | 冻结 |
| 报文 `schema_version` | **2.0.0，未随文档升版** —— v2.1 没有动任何线上字段，见下 |
...
| 版本 | 日期 | 改了什么 | 要不要动 `schema_version` |
|---|---|---|---|
| v2.0 | D3 | 评审通过，24 条决议落地，接口重新冻结 | 升到 2.0.0 |
| v2.1 | 2026-09-10 | §6.4：`delta_conf` 由判据降为记录项，新增派生指标 `l2_yield` | **不动，仍是 2.0.0** |

**v2.1 为什么不升 `schema_version`。**它改的是「达标判据怎么算」，不是报文格式：
`l2_yield` 是 `tools/run_all.py` 在小结里现算的派生量，**没有进
`evidence_package.schema.json` 的 `gain`**（那里仍然只有 `delta_conf` /
`pixel_density_ratio` / `verify_success` 三个）。冻结规则约束的是字段名、枚举值、
指令白名单、数值范围，这次一个都没碰，所以线上版本号保持 2.0.0。
`patrol/__init__.py` 的 `SCHEMA_VERSION = "2.0.0"` 是对的，**不要去"修"它**。

**文件名仍是 `-v2.0.md`**，没有跟着改：改名要动所有引用它的链接，而文档编号
（ICD-RK3576-PATROL）本来就不含版本号，版本以本表为准。
...
| 字段 | 类型 | 计算 | 目标 |
|---|---|---|---|
| `delta_conf` | float | `after.confidence − before.confidence` | **仅记录，不作判据**（见下） |
...
| `l2_yield` | 派生 | 产出 L2 读数的证据包 / 总包数 | > 80 % |

#### `delta_conf` 为什么从判据降为记录项（v2.1 修订，2026-09-10）

**原判据 `> +0.25` 建立在一个被实测推翻的假设上。**它假设巡航态对目标是
不确定的（合成基线：远景 ~0.4 → 复核确认 ~0.95，Δ≈+0.5）。真权重接进来之后
这个前提不成立：巡航态置信度实测 0.85–0.93，复核态 0.90–0.92，
**Δconf 的数学上限只剩 ~0.1**，无论怎么训模型都够不到 +0.25。

这不是模型不够好，恰恰是模型太好——一级检出在远景就已经足够自信。

三条独立证据：

| 来源 | Δconf 实测 |
|---|---|
| 甲，真权重 yolo（`--seconds 330`） | +0.0072 |
| 甲，合成检测器同口径对照 | +0.0130 |
| 组长，本机三轮（真权重 + 几何先验匹配） | +0.0013 / −0.0246 / +0.0602 |

**合成检测器也够不到**——所以它不是"换了真模型才不达标"，是这条判据本身
从来就没成立过，只是此前没有真权重去检验它。

复核的增益是真实的，只是不体现在置信度差上。改用下面三项，都有实测支撑：

1. **误报抑制**：`FALSE_ALARM` 组 0.93 → 0.0（复核变焦后框直接检不出）
2. **读数产出**（新增 `l2_yield`）：复核态 7/7 产出 L2 读数，巡航态 0/7
   ——这是"停车、对准、变焦、再看一眼"最直接的兑现
3. **像素密度比**：2.2–2.9×，已在表内，保留为判据

`delta_conf` 仍然逐包记录、仍然写进 manifest，只是不再作为达标判据。
**删掉它是错的**：它是诊断量——真出现巡航 0.4 / 复核 0.95 那种情形时，
它会告诉你这一路检测器在远景上不自信，那是模型该改进的信号。

> 修订依据：任务书原文「本课题存在……相互制约的指标，**要求学生在方案设计中
> 明确取舍**」。此处即一次明确取舍：判据与实测冲突时，改判据而不是改数字，
> 并把冲突的证据链留在文档里。

三项由 `uploader` 在打包时计算并写死在 manifest 里，不由云端二次计算。云端重算会引入版本不一致的风险，而这三个数字是要写进答辩材料的。

`delta_conf` 对 `FALSE_ALARM` 会是负值，这是正常的：复核把一个 0.41 的误检压到 0.05，`delta_conf = −0.36`。统计时按 `verdict.result` 分组——**分组这件事本身仍然必要**，混在一起算任何一项都会被误报组拉偏。这一点在做统计脚本时最容易搞错，把两组混在一起算会让均值接近零，看上去像是复核没起作用。

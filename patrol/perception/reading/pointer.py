...
处理链：
    ...
    → 极坐标展开（warpPolar，行 = 角度，列 = 半径）
    → 径向带 0.28R–0.70R 内逐角度算暗覆盖率，峰值定指针方向
    → 峰值附近逐半径环算角质心、取中位数，细化到亚度级 → 标度变换

三个关键设计（细节见 _needle_angle）：

1. **靠"贯穿径向带"认出指针。**展开后指针是一条贯穿整个径向带的暗条；刻度
   自 0.68R 起、数字是孤立块，都给不出贯穿式的覆盖。配重尾巴只到 0.18R，
   落在带外，所以峰值只有一个，不存在 180° 歧义。

2. **逐环角质心取中位数，且全程保留亚像素信息。**指针靠近轴帽处张角大、
   尖端处窄，沿半径求和会被内圈的宽裙边带偏；逐环算再取中位数，个别被数字
   或刻度污染的环成了离群点被挡掉。插值用 warpPolar 的双线性、覆盖率用软阈值，
   手写整数像素采样或硬二值都会把质心量化成台阶。

3. **b/a < 0.85 判为视角过斜，置信度打折并带出 axis_ratio。**方案书 §6.1.4：
   此时补偿后残余误差仍不可忽略，应由状态机下发云台角度调整重新拍摄——用控制
   手段替代软件补偿，这正是本课题"主动测量"的体现。

给了分割模型时，只把"哪些像素是针"换成它的概率图，后面几步照旧；分割没给出
可用掩膜就退回纯几何。当前配置不启用分割（比选结论见 docs/指标汇总表.md）。
"""
